...
for (i in 1:m) 
  plot(x = 0:n, y = dbinom(x = 0:n, size = n, prob = theta[i]), type = "h", lwd = 4, col = col[i], xlab = "y", ylab = "Densidad", main = bquote(theta==.(round(theta[i],2))))
```
...
knitr::include_graphics("teorema_bayes.png")
```
...
legend("topright", legend = c("Posterior","Previa"), col = c(4,2), lty = 1, lwd = 2, bty = "n")
```
...
La **probabilidad** es una parte de las Matemáticas dedicada a la **cuantificación de la incertidumbre**.
...
La distribución posterior $p(\boldsymbol{y}\mid\boldsymbol{\theta})$ es función de $\boldsymbol{\theta}$ una vez que se observa $\boldsymbol{y}$. **R. Fisher** popularizó esta idea y la llamó **función de verosimilitud**, 
$$
\ell(\theta\mid\boldsymbol{y}) = c\,p(\boldsymbol{y}\mid\boldsymbol{\theta}),
$$ 
