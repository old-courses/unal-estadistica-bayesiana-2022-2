...
También, la **distribución predictiva posterior** es **Binomial Negativa** con parámeros $a+s$ y $b+n$, i.e., $y^*\mid \boldsymbol{y}\sim \textsf{BN}(a+s,b+n)$:
$$
...
rownames(tab) <- c("Bayesiana", "Frec. Asintótico", "Frec. Boostrap")
print(round(tab, 3))
```
